	...
	// read
		...
		last() {
			...
			return new Promise((resolve, reject) => {
				...
				that.db.get(_sSelectQuery + " ORDER BY triggers.id DESC LIMIT 0,1;", [], (err, row) => {
		...
		search(data) {
			...
			return new Promise((resolve, reject) => {
				...
				that.db.all(query + " ORDER BY triggers.name ASC;", options, (err, rows) => {
		...
		delete (trigger) {
			...
			if (!trigger) {
				...
				return new Promise((resolve, reject) => {
					...
					that.db.run("DELETE FROM triggers WHERE id = :id;", { ":id" : trigger.id }, (err) => {
