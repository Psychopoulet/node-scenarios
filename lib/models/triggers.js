...
module.exports = class DBTriggers extends require(require("path").join(__dirname, "..", "main.js")).abstract {
			...
			return new Promise((resolve, reject) => {
				...
				if ("object" === typeof data && null !== data) {
					...
					if (data.action && data.action.id) {
						...
						query += " INNER JOIN actionstriggers ON actionstriggers.id_action = :id_action";
						options[":id_action"] = data.action.id;
					...
					if (data.id) {
						...
						options[":id"] = data.id;
							...
							query += " AND triggers.id IN(" + data.ids.substring(1, data.ids.length - 1) + ")"
						}
					else if (data.code) {
	...
	// write
		...
		linkToAction (action, trigger) {

			if (!action) {
				return Promise.reject("There is no action data.");
			}
				else if (!action.id) {
					return Promise.reject("There is no valid action data.");
				}
			else if (!trigger) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("INSERT INTO actionstriggers (id_action, id_trigger) VALUES (:id_action, :id_trigger);", {
						":id_action": action.id,
		...
		unlinkToAction (action, trigger) {
			...
			if (!action) {
				...
				return new Promise((resolve, reject) => {
					...
					this.db.run("DELETE FROM actionstriggers WHERE id_action = :id_action AND id_trigger = :id_trigger;", {
						":id_action": action.id,
						":id_trigger": trigger.id
					}, (err) => {
